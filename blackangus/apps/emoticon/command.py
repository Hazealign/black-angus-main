...
class EmoticonCommandApp(PresentedResponseApp):
    ...
    async def parse_command(self, context: discord.Message) -> Optional[Dict[str, Any]]:
        def get_equivalents(x: str):
            return ['-e', '--equivalents'].__contains__(x)
        ...
        if (
            ...
            or '-h' in parsed
            or '--help' in parsed
            or ['도움', 'help'].__contains__(parsed[0])
        ):
            return {'help': True}

        if ['목록', 'list'].__contains__(parsed[0]):
            return {
                'help': False,
                'action': 'list',
        ...
        if ['추가', 'add', 'create'].__contains__(parsed[0]):
            if len(parsed) < 3:
                return {'error': True}
        ...
        if ['검색', 'search'].__contains__(parsed[0]):
            if len(parsed) < 2:
                return {'error': True}
        ...
        if ['복제', 'duplicate'].__contains__(parsed[0]):
            if len(parsed) < 3:
                return {'error': True}
        ...
        if ['삭제', 'delete', 'remove'].__contains__(parsed[0]):
            if len(parsed) < 2:
                return {'error': True}

            first_equivalents = len(parsed) == 3 and get_equivalents(parsed[1])
            last_equivalents = len(parsed) == 3 and get_equivalents(parsed[2])
        ...
        try:
            ...
            if action == 'search':
                ...
                return None, Embed(
                    ...
                    name='목록',
                    value=', '.join(map(lambda x: x.name, emoticons)),
                    inline=False,
                )
